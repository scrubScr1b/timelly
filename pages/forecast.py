...
# --- Plot Perbandingan Semua Model ---
st.subheader("Perbandingan Semua Forecast")
fig = go.Figure()
fig.add_trace(go.Scatter(x=train['date'], y=train['total_sales'], name='Train'))
fig.add_trace(go.Scatter(x=test['date'], y=test['total_sales'], name='Test (Actual)', line=dict(color='blue')))
fig.add_trace(go.Scatter(x=test['date'], y=sarimax_forecast, name='SARIMAX', line=dict(color='red')))
fig.add_trace(go.Scatter(x=test['date'], y=blstm_forecast, name='BLSTM', line=dict(color='green')))
fig.add_trace(go.Scatter(x=test['date'], y=hybrid_dynamic, name='Hybrid Proximity', line=dict(color='purple', dash='dot')))
fig.update_layout(width=1300, height=600, title="Compare Forecast Models")
st.plotly_chart(fig)